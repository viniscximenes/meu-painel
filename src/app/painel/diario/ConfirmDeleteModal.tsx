...
export default function ConfirmDeleteModal({ registro, onFechar, onApagado, onErro }: Props) {
  ...
  return (
    ...
    >
      <div
        className="animate-fadeInScale w-full max-w-md rounded-2xl border overflow-hidden flex flex-col"
        style={{
          background: 'linear-gradient(180deg, rgba(17,24,39,0.99) 0%, rgba(8,12,20,0.99) 100%)',
          borderColor: 'rgba(239,68,68,0.25)',
          boxShadow: '0 32px 96px rgba(0,0,0,0.85), 0 0 0 1px rgba(239,68,68,0.06)',
        }}
      >
        {/* Header */}
        <div
        ...
        >
          ...
          <button
            ...
            style={{
              background: pending ? 'rgba(239,68,68,0.15)' : 'rgba(239,68,68,0.18)',
              color: '#f87171',
              border: '1px solid rgba(239,68,68,0.30)',
              minWidth: '110px',
              justifyContent: 'center',
            }}
            onMouseEnter={(e) => { if (!pending) (e.currentTarget as HTMLElement).style.background = 'rgba(239,68,68,0.28)' }}
            onMouseLeave={(e) => { if (!pending) (e.currentTarget as HTMLElement).style.background = 'rgba(239,68,68,0.18)' }}
          >
            {pending ? (
