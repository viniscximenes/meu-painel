...
import { TrendingUp, Target, AlertTriangle, ChevronDown, Check, X, Zap } from 'lucide-react'
...
const CENARIOS = [
  {
    label: 'Meta Mínima',
    desc:  'RV mínima elegível',
    retracao: 66, tmaMmss: '12:11', abs: 5,   indisp: 14.5, faltas: 0,
    color: '#fbbf24',
  },
  {
    label: 'Meta Confortável',
    desc:  'RV com bônus',
    retracao: 70, tmaMmss: '10:00', abs: 2,   indisp: 11,   faltas: 0,
    color: '#4ade80',
  },
  {
    label: 'Máximo Possível',
    desc:  'RV máxima teórica',
    retracao: 80, tmaMmss: '08:00', abs: 0,   indisp: 8,    faltas: 0,
    color: '#c9a84c',
  },
]
  ...
  return (
    <div style={{ marginBottom: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
        ...
        <span style={{
          fontSize: '15px', fontWeight: 700, fontFamily: 'var(--ff-display)',
...
function ResultadoSimulado({ r, config, pedidos }: { r: SimResult; config: RVConfig; pedidos: number }) {
  ...
  const linhas: { label: string; valor: number; ok: boolean; detalhe?: string }[] = [
    ...
    <Card style={{ borderColor: r.elegivel ? 'rgba(201,168,76,0.2)' : 'rgba(248,113,113,0.15)', position: 'sticky', top: '80px' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '14px' }}>
        <p style={{
          ...
          color: r.elegivel ? 'var(--gold)' : '#f87171',
      ...
      <div style={{ borderTop: '1px solid rgba(201,168,76,0.08)', paddingTop: '12px' }}>
        ...
        <div style={{ borderTop: '1px solid rgba(201,168,76,0.08)', marginTop: '8px', paddingTop: '8px', display: 'flex', justifyContent: 'space-between' }}>
          ...
          <span style={{
            fontSize: '14px', fontWeight: 700, fontFamily: 'var(--ff-display)',
            color: r.elegivel ? 'var(--gold)' : '#f87171',
          }}>
            {formatBRL(r.rvTotal)}
          </span>
...
function Simulador({ p, txRetracao, setTxRetracao, tmaInput, setTmaInput, absVal, setAbsVal, indispVal, setIndispVal, faltas, setFaltas, simResult }: SimuladorProps) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '24px' }}>
        ...
        <div>
          ...
          <SliderKpi
            ...
            marker={config.absMaximo}
            markerLabel={`${config.absMaximo}% (limite)`}
            formatVal={v => `${v.toFixed(1)}%`}
          ...
          <SliderKpi
            ...
            value={indispVal}
            onChange={setIndispVal}
            min={0} max={25} step={0.1}
            marker={config.indispLimite}
            markerLabel={`${config.indispLimite}% (limite)`}
            formatVal={v => `${v.toFixed(1)}%`}
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
              ...
              <span style={{
                fontSize: '15px', fontWeight: 700, fontFamily: 'var(--ff-display)',
                color: faltas >= 2 ? '#f87171' : faltas === 1 ? '#fbbf24' : '#4ade80',
              }}>
                {faltas}
              </span>
            </div>
        ...
        {/* Resultado */}
        <ResultadoSimulado r={simResult} config={config} pedidos={p.pedidosAtual} />
      </div>
    </Card>
  )
}
...
function CenariosRapidos({ p, onSelect }: CenarioProps) {
  return (
    <Card>
      <SectionHeading>Cenários Rápidos</SectionHeading>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '10px' }}>
        {CENARIOS.map(c => {
          const r = calcSimulado(c.retracao, mmssParaSeg(c.tmaMmss), c.abs, c.indisp, c.faltas, p.pedidosAtual, p.ticketAtual, p.rvConfig)
          return (
            <button
              key={c.label}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 700, color: c.color, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                  {c.label}
                </span>
              </div>
        ...
        <div style={{ paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                {config.retracaoFaixas.map(f => {
                  ...
                  return (
                    <tr key={f.min} style={{
                      ...
                      <td style={{ padding: '5px 8px', fontSize: '12px', color: ativa ? 'var(--gold)' : 'var(--text-muted)', fontWeight: ativa ? 700 : 400 }}>
                        ≥ {f.min}%
                      </td>
                      <td style={{ padding: '5px 8px', fontSize: '12px', color: ativa ? 'var(--gold)' : 'var(--text-secondary)', fontWeight: ativa ? 700 : 400, textAlign: 'right' }}>
                        {formatBRL(f.valor)}
                        {ativa && <span style={{ fontSize: '9px', marginLeft: '4px', opacity: 0.7 }}>← atual</span>}
                      </td>
                    </tr>
                  )
                })}
                <tr>
                  <td style={{ padding: '5px 8px', fontSize: '12px', color: 'var(--text-muted)' }}>
                    {'< '}{config.retracaoFaixas[config.retracaoFaixas.length - 1]?.min ?? 57}%
                  </td>
                  <td style={{ padding: '5px 8px', fontSize: '12px', color: 'var(--text-muted)', textAlign: 'right' }}>
                    {formatBRL(0)}
                  </td>
                </tr>
              </tbody>
          ...
          {config.ticketFaixas.length > 0 && (
            <div>
              ...
              <p style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '8px' }}>
                Requer TX Retenção ≥ {config.ticketMinRetracao}%
              </p>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <tbody>
          ...
          <div>
            ...
            <p style={{ fontSize: '12px', color: 'var(--text-muted)', lineHeight: 1.6 }}>
              {formatBRL(config.bonusValor)} se TX Retenção ≥ {config.bonusRetracaoMinima}%
              {' '}e Indisponibilidade ≤ {config.bonusIndispMaxima}%
            </p>
          </div>
...
export default function CalculadoraRVClient(p: CalculadoraRVProps) {
  ...
  const simResult = useMemo(
    () => calcSimulado(txRetracao, tmaSeg, absVal, indispVal, faltas, p.pedidosAtual, p.ticketAtual, p.rvConfig),
    [txRetracao, tmaSeg, absVal, indispVal, faltas, p.pedidosAtual, p.ticketAtual, p.rvConfig],
  )
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <Fade delay={120}>
          <CenariosRapidos p={p} onSelect={applyScenario} />
        </Fade>
