...
import { Shield, Calendar, Activity, Clock, Flame, Check } from 'lucide-react'
import type { MetaGestorConfig } from '@/lib/kpi-utils'
import { salvarMetaGestorAction } from '@/app/painel/metas/actions'
...
function CardTxRetencaoGestor({ config, onToast }: { config?: MetaGestorConfig; onToast: (t: Toast) => void }) {
  ...
  function save() {
    ...
    startTransition(async () => {
      const res = await salvarMetaGestorAction('tx_ret_bruta', {
...
function CardMenorMelhorGestor({
  ...
  config?: MetaGestorConfig; onToast: (t: Toast) => void
}) {
  ...
  function save() {
    ...
    if (isNaN(vv) || vv <= 0) { onToast({ tipo: 'erro', msg: 'Valor verde inválido.' }); return }
    startTransition(async () => {
      const res = await salvarMetaGestorAction(kpiKey, {
        modo: 'limiar_global', verde_op: '<=', verde_valor: vv,
...
function CardChurnGestor({
  ...
  function save() {
    ...
    if (!/^[A-Z]{1,3}$/.test(col)) {
      onToast({ tipo: 'erro', msg: 'Informe uma letra de coluna válida (ex: K).' })
      return
    }
    startTransition(async () => {
      const res = await salvarMetaGestorAction('churn', {
        modo: 'coluna_individual', coluna_meta: col,
        verde_op: null, verde_valor: null, amarelo_op: null, amarelo_valor: null,
      })
      onToast(res.ok ? { tipo: 'ok', msg: 'Churn salvo.' } : { tipo: 'erro', msg: res.erro ?? 'Erro.' })
    })
  }

  return (
    <div style={CARD}>
      <CardHeader icon={Flame} titulo="Churn" subtitulo="Meta individual por gestor" />
      <p style={{ fontFamily: FF_DM, fontSize: '12px', color: '#72708F', lineHeight: 1.6, marginBottom: '16px' }}>
        Cada gestor tem sua meta de cancelados definida em uma coluna da aba{' '}
        <strong style={{ color: '#A6A2A2' }}>KPI GESTOR</strong>.
...
export default function MetasGestorBloco({ configs }: { configs: Record<string, MetaGestorConfig> }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <CardTxRetencaoGestor config={configs['tx_ret_bruta']} onToast={showToast} />
        <CardMenorMelhorGestor
        ...
        />
        <CardChurnGestor config={configs['churn']} onToast={showToast} />
      </div>
    </>
  )
}
